    ...
    /**New Relic Logs */
    let newRelicObject = {};
    const newRelicLogCategory = "newReliclogError";
    const newRelicLogTypeCategory = "eCWChormeExtension";
...
athena = {
    ...
    ecwLog: function(type, message) {
		const uuidKey = `ecw_uuid_${type}`;
                ...
                if (!athena.container().get()) {
                    ...
                    if (!$container.length) {
                        ...
                        $container.load(chrome.runtime.getURL('html/container.html'), null, function () {
                            athena.log('Infera results frame loaded.');
    ...
    openSlider: function (data) {
        ...
        window.addEventListener(responseEvent, function () {
            ...
            athena.log('Custom event received: ' + responseEvent);
        });

        athena.log('Dispatching custom event: Athena:message ' + JSON.stringify(data));
        ...
        function loadTimer() {
            ...
            if (url.match('progressnotes')) {
                ...
                item.off().on('click', async function (e) {
                    // console.log("opening after load click");
                    glo_val = 1;
                    athena.container().frames().hide();
                    ...
                    // console.log("removing local storage submission & rejection");                        
        ...
        if (!athena.isAthenaPreview()) {
            ...
            window.onpopstate = function (event) {
                if (event) {
                    ...
                    let url = window.location.href;
                    // console.log(url);
                    boxes = Array.from(document.getElementsByClassName('infera-overlay-tab drawer-tab position'));
                    if (url.match('progressnotes') && boxes.length) {
                        athena.closeInferatab();
                    }

                    /**care gap tag select and hide */
                    boxes2 = Array.from(document.getElementsByClassName('infera-overlay-tab2 drawer-tab2 position2'));
                    if (url.match('progressnotes') && boxes2.length) {
                        caregap.closeInferatab2();
                    }
            ...
            let glo_val = 0;
            window.onload = clickTimer;

            if (glo_val == 1) {
                window.onclick = clickTimer;
            }
            ...
            function clickTimer() {
                ...
                $('div#infera-container').on('click', function () {
                ...
                if (glo_val == 1) {
                    ...
                    t = setTimeout(loadTimer, 5000);
                }

            }

        } else {
            // console.log('welcome to Athena')
        }
    ...
    onVerifyloadTimer: function () {
        ...
        try {
            ...
            let patient_obj = document.getElementById('toppanelcontainer').attributes.patientobj.value;
            //// console.log('patient Obj: ',patient_obj); $('#doctorID').val()
            patient_obj = JSON.parse(patient_obj);
            patient_obj.physicianId = null;
            patient_obj.main_user = $('#doctorID') ? $('#doctorID').val() : '';
            ...
            makeRequest('POST', 'https://myinfera.inferscience.com/api/verify-eula', patient_obj).then(function (data) {
                ...
                if (!data.error) {
                    ...
                    if (data.success) {
                        ...
                        t = setTimeout(athena.onloadTimer, 2000);
                    }
                } else {
                    localStorage.removeItem('infera-view');
    ...
    onViewloadTimer: async function () {
        ...
        try {
            ...
            patient_obj = JSON.parse(patient_obj);
            patient_obj.physicianId = null;
            patient_obj.main_user = $('#doctorID').val();
            patient_obj = JSON.stringify(patient_obj);
            ...
            // console.log("wanted to view more", err)
    ...
    loaderLoading: function () {
        ...
        athena.log("loading start " + Date.now())
    ...
    onloadTimer: function () {
        ...
        function checkModalTimer(counter = 0, maxTimer) {
            ...
            while (counter < maxTimer) {
                //// console.log('counter'+counter,maxTimer);
                setTimeout(function () {
        ...
        try {
            ...
            let enc_id = document.getElementById('toppanelcontainer').attributes.encid.value;
            //// console.log('encounter ID: ',enc_id);
            patient_obj = document.getElementById('toppanelcontainer').attributes.patientobj.value;
            patient_obj.physicianId = null;
            ...
            */
            ...
            //// console.log(text);
            //// console.log('patient obj full name: ',JSON.parse(patient_obj).FULL_NAME);
            //// console.log('onload patient full name: ',document.querySelector("#pat-details > div > div.relative.pat-wrapper > div.pat-history > div.pat-details > h2 > span:nth-child(2)").textContent);

            if (document.querySelector("#pat-details > div > div.relative.pat-wrapper > div.pat-history > div.pat-details > h2 > span:nth-child(2)").textContent != JSON.parse(patient_obj).FULL_NAME) {
                    ...
                    makeRequest('POST', "https://myinfera.inferscience.com/api/data", patient_obj).then(function (data) {
                        ...
                        xhttp.onreadystatechange = function () {
                            if (this.readyState == 4 && this.status == 200) {
                                ...
                                athena.ecwLog('Type:56',myObject);        
                                // myObject = {};
                                ...
                                /***
                                 * Hcc, RxHcc, Claims, User Diagnosis code count
                                 */
                                //User Diagnosis
                                let type_user = [];
                                $(".type-patd td div[data-value]").each(function() {
                                    let userDataValue = $(this).attr('data-value');
                                    if (!userDataValue.startsWith('patd-')) {
                                        type_user.push(userDataValue);
                                    }
                                });
                                //Rxhcc
                                let type_rx = [];
                                $(".type-rx td div[data-value]").each(function() {
                                    let rxDataValue = $(this).attr('data-value');
                                    if (!rxDataValue.startsWith('clu-')) {
                                        type_rx.push(rxDataValue);
                                    }
                                });                                

                                //Claims
                                let type_claims = [];
                                $(".type-claims td div[data-value]").each(function() {
                                    let claimsDataValue = $(this).attr('data-value');
                                    if (!claimsDataValue.startsWith('claims-')) {
                                        type_claims.push(claimsDataValue);
                                    }
                                });
                                //HCC
                                let type_cms = [];
                                $(".type-cms td div[data-value]").each(function() {
                                    let cmdDataValue = $(this).attr('data-value');
                                    if (!/^\d+$/.test(cmdDataValue) && !cmdDataValue.startsWith('clu-')) {
                                        type_cms.push(cmdDataValue);
                                    }
                                });

                                //Create multidimension array for all the codes with key and value
                                let codes = {
                                    'user_diagnosis': type_user,
                                    'rxhcc': type_rx,
                                    'claims': type_claims,
                                    'hcc': type_cms
                                };
                                myObject.resultCodes = codes;                  
                                athena.ecwLog('Type:60',myObject);        
                                myObject = {};
                                ...
                                */
                                        ...
                                        xhttp.onreadystatechange = function () {
                                            if (this.readyState == 4 && this.status == 200) {
                                                ...
                                                $("#hcc-preferences-form > div.modal-footer > input").on('click', function (e) {
                                                    ...
                                                    myObject.preferences = oriVal.preferences.hcc_show_existing_codes == 1 ? 'TYPE:81' : 'TYPE:82';
                                                    myObject.EHR = 'ecw';                                                    
                                                    athena.ecwLog(oriVal.preferences.hcc_show_existing_codes == 1 ? 'TYPE:81' : 'TYPE:82', myObject);
                                        ...
                                        xhttp.onreadystatechange = function () {
                                            if (this.readyState == 4 && this.status == 200) {
                                                ...
                                                var codeSelect2onSelect = function ($el) {
                                                    ...
                                                    /*
                                                    // close button icon .....---------------------------------------------
                                                    type = $el[0].dataset.select2Id;
                                                    hcc_idx = type.indexOf('hcc');
                                                    companion_idx = type.indexOf('companion');
                                                    related_idx = type.indexOf('related');
                                                    if(! (hcc_idx < 0)){
                                                        // console.log('hcc codes type')
                                                        span_children_len = $("#add-code-modal > div:nth-child(3) > div > span > span.selection > span > ul")[0].children.length
                                                        // console.log('span_children_len: ', span_children_len)
                                                        for(let i=0;i<span_children_len-1;i++){
                                                            span_condition = $("#add-code-modal > div:nth-child(3) > div > span > span.selection > span > ul")[0].children[i].innerHTML;
                                                            // console.log('span condition', span_condition)
                                                            if((span_condition.indexOf('&nbsp')<0)){
                                                                // console.log('added nbsp')
                                                                let j = (i+1);
                                                                let x = $('<span>&emsp;</span>').insertAfter('#add-code-modal > div:nth-child(3) > div > span > span.selection > span > ul > li:nth-child('+j+') > span');
                                                                // console.log('x',x)
                                                                //span_add = $("#add-code-modal > div:nth-child(3) > div > span > span.selection > span > ul")[0].children[i].children[0].outerHTML;
                                                                //// console.log('span add', span_add)
                                                                //$("#add-code-modal > div:nth-child(3) > div > span > span.selection > span > ul")[0].children[i].children[0].outerHTML = span_add + '&nbsp&nbsp&nbsp;'; 
                                                            }														
                                                        }
                                                    	
                                                    }else if(! (companion_idx < 0)){
                                                        // console.log('companion codes type')
                                                    }else{
                                                        // console.log('related codes type')
                                                    }
                                                    */
                                                    try {
                                                        var el = $el[0];
                                ...
                                $('button[data-value="emr"]').on('click', function () {
                                    ...
                                    if (flag == 1) {
                                        ...
                                        */
                                        ...
                                        selectedCodes = selectedCodes.filter(code => code.length < 8);
                                        console.log("Final submit plan of care: ", selectedText)
                                        console.log("Final submit codes: ", selectedCodes)
                                        if(!(selectedCodes.length == 0 && selectedText.length == 0)){
                                            athena.addCode_assessment_submittion(selectedCodes, selectedText) // fast submission
                                            $messages = $('.messages', $('.container'));
                                            $messages.empty();
                                            $('button[data-behavior="submit"]').prop('disabled', true)
                                            selectedCodes = [];
                                            ...
                                            alert('Please select main codes before submitting to EMR.')
                                        }                                        
                                ...
                                $('button[data-value="reject-direct"]').on('click', function () {
                                    ...
                                    makeRequest('POST', 'https://myinfera.inferscience.com/api/post-rejected-data', getRejected).then(function (data) {
                                        ...
                                        if (data.success) {
                                            ...
                                            selectedText = [];
                                            selectedCodeText = [];
                                            // Your selected items were successfully flagged as rejected.
                                            athena.showMessages($messages, [
                                                'Your selected item(s) were successfully flagged as rejected.'
                                                ],
                                                'success'
                                            );
                                            
                                            localStorage.setItem('rejection', "rejection_completed");
                                            localStorage.removeItem('submission');
                                            setTimeout(function(){       
                                                console.log('after rejection-direct')                             
                                                $messages = $('.messages', $('.container'));	
                                                    ...
                                                    if (finalArray[3].length > 0) {
                                                        ...
                                                        makeRequest('POST', 'https://myinfera.inferscience.com/api/post-info-rejected-data', finalArrayInfo).then(function (data) {
                                                            ...
                                                            if (data.success) {
                                                                ...
                                                                localStorage.removeItem('submission');                                                                
                                                                setTimeout(function(){   
                                                                    console.log('after rejection-info')                                                              
                                                                    $messages = $('.messages', $('.container'));	
                                                                    $messages.removeClass('toast-msg')						
                                                                    $messages.empty();
                                                                    localStorage.removeItem('submission');
                                ...
                                $('button.select-all').on('click', function () {
                                    ...
                                    if (button_click_off == 1) {
                                        ...
                                        $('button.select-item.primary-code[data-value="false"]:not(.hidden)', 'table#hcc-codes-table').click();
                                        button_click_off = 0;
                                    } else {
                                ...
                                $('table#hcc-codes-table').on('click', 'button.more-info', function () {
                                    ...
                                    if (code) {
                                        ...
                                        api_url = api_url + JSON.parse(patient_obj).ID + '/' + $('#doctorID').val();
                                    } else {
                                        api_url = api_url + '/' + JSON.parse(patient_obj).ID + '/' + $('#doctorID').val();
                                ...
                                $('table#hcc-codes-table').on('click', 'button.select-item', function () {
                                    ...
                                    //// console.log('container: ',$('.main-select-item', $row)[0].className.includes('hidden'));
                                    //// console.log('select plan of care',$selectPlanOfCare)
                                    if (!$('.main-select-item', $row)[0].className.includes('hidden')) {
                                            ...
                                            if (active) { // Enable selection of the related text. --}}
                                                ...
                                                if (related.length > 0) {
                                                    ...
                                                    $.each(related, function (idx, code) {
                                                        ...
                                                        var $relatedButton = $('button.select-item[data-value="true"]:not(.hidden)',
                                    ...
                                    if (!active) {
                                        ...
                                        var $selected = $('button.select-item[data-value="false"]:not(.hidden)', $row);
                                        ...
                                        $selected = $('button.select-item[data-value="false"]:not(.hidden)', $('div[data-behavior="code"]', $row));

                                        if ($selected.length < 1) {
                                            $('button.select-item[data-value="false"]:not(.hidden)', $textContainer).click();
                                    ...
                                    $('button.select-item[data-value="' + active + '"]:not(.hidden)',
                                    ...
                                    // console.log('selected codes', selectedCodes);
                                    // console.log('selected codes text', selectedCodeText);
                                    ...
                                    var deselect = $('button.select-item.primary-code.hidden[data-value="true"]').length;
                                    ...
                                    */
                                    if (!($('button[data-value="save"]').length || $('button[data-behavior="clear-saved-result"]').length)) {
                                        ...
                                        var deselect = $('button.select-item.primary-code.hidden[data-value="true"]').length + $('button.select-item.secondary-code.hidden[data-value="true"]').length;
                                        var actualselect = select1 - deselect;
                                ...
                                $('table#hcc-codes-table').on('mousedown', '.popover', function(e) { e.preventDefault(); });
                                */
                                /*
                                setTimeout(function(){
                                    //// console.log("td cl down started")
                                    $('td.tg-cl-down').each(function(i,e){							
                                        var el1 = $(e);
                                        //// console.log('toggle arrows')
                                        //// console.log(el1,el1[0].clientHeight)
                                        if(el1[0].clientHeight<50){
                                            el1.on('click', function(){
                                                //// console.log('if css height',el1[0].clientHeight)									
                                                el1.toggleClass('tg-cl-down').toggleClass('tg-cl-up1');
                                            })								
                                        }
                                        else if(el1[0].clientHeight>85){
                                            el1.on('click', function(){
                                                //// console.log('else css height',el1[0].clientHeight)									
                                                el1.toggleClass('tg-cl-down').toggleClass('tg-cl-up2');
                                            })								
                                        }else{
                                            el1.on('click', function(){
                                                //// console.log('else css height',el1[0].clientHeight)									
                                                el1.toggleClass('tg-cl-down').toggleClass('tg-cl-up');
                                            })								
                                ...
                                */
                                ...
                                if (!toggleData) {
                                    ...
                                    $('i.toggle-row').each(function (i, e) {
                                        ...
                                        if (el[0].offsetParent) {
                                            ...
                                            el[0].offsetParent.className = 'vertical-class-down';
                                        ...
                                        el.on('click', function () {
                                            ...
                                            if (el[0].offsetParent.className.includes('class-down')) {
                                                el[0].offsetParent.className = 'vertical-class-up';
                                            } else {
                                                el[0].offsetParent.className = 'vertical-class-down';
                                            }
                                    ...
                                    if (!data.success) {
                                        ...
                                        if (getNewResult) {
                                            ...
                                            if ($('button[data-behavior="clear-saved-result"]').length) {
                                                $('button.remove-rejection').addClass('hide_results')
                                            }
                                        }
                                ...
                                document.querySelectorAll('.help-icon').forEach(function (el) {
                                    el.addEventListener('click', function (e) {
                                        ...
                                        pop.innerHTML = `
                                            <div class="popover-inner">${el.getAttribute('data-content')}</div>
                                            <div class="popover-arrow"></div>
                                        `;

                                        // append to same parent so it scrolls WITH the icon
                                        const parent = el.parentElement;
                                        parent.style.position = "relative"; // ensure relative context
                                        ...
                                        pop.style.left = (rect.left - parentRect.left - 122) + "px";
                                        pop.style.top  = (rect.top - parentRect.top - pop.offsetHeight - 8) + "px";
                                    });
                                });
                                ...
                                //     if (!e.target.closest('.help-icon')) {
                                //         console.log('scroll detected anywhere - removing popovers');
                                //         document.querySelectorAll('.custom-popover').forEach(p => p.remove());
                                //     }
                                // }, true); // <-- useCapture = true (important)
            ...
            makeRequest('POST', "https://myinfera.inferscience.com/api/get-eula", patient_obj).then(function (data) {
                ...
                xhttp.onreadystatechange = function () {
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        $('a.print-button-eula').addClass('ptr-cursor');
    ...
    formatNewProcessedCodes: function (data, status) {
        ...
        $('div[data-tick="tick"]').each(function (i, e) {
            ...
            $text.find('button[data-value="true"]').attr('disabled', 'disabled').removeClass('hidden');
            $text.find('button[data-value="false"]').removeAttr('disabled').addClass('hidden');
    ...
    formatRemoveRejectedCodes: function (data, status) {
        ...
        function setItemStatus($parent, status) {
            ...
            $parent.find('i.status-' + status).addClass('hidden');
        };
        ...
        $.each(data, function (i, val) {
            ...
            $('div[data-value="' + val + '"] > table > tbody > tr > td:nth-child(1)').append('<button class="btn btn-orange text-uppercase select-item primary-code main-select-item bt" data-value="true" data-role="HCC">Select</button><button class="btn btn-primary text-uppercase select-item primary-code main-select-item hidden bt" data-value="false" data-role="HCC">Deselect</button>');
    ...
    addCode_assessment_submittion: function (data_obj, selected_text_obj) {
        ...
        $('button.select-item').addClass('disabled')
        $('button.more-info').addClass('disabled')
        console.log("assessment_submittion start ")
        ...
        if (url.match('progressnotes')) {
            ...
            /*
            setTimeout(function(){
                // console.log('settimeout function',Date.now())
                $modal = $('#loading-modal');
            ...
            */
            let smart_checked = false; 
            document.querySelector("#btnFilter").click()
            setTimeout(function(){
                smart_Search_Checkbox = $("#chkSmartSearch")[0].checked
                if (smart_Search_Checkbox){
                    console.log('unchecking smart search')
                    smart_checked = true;
                    document.querySelector("#chkSmartSearch").click()
                }else{
                    console.log('smart search already unchecked')                    
                }
                // start assessment submission process after smart search is unchecked
                t = setTimeout(onloadProgressTimer, 1100);
            },500)

            // t = setTimeout(onloadProgressTimer, 10);

            function onloadProgressTimer() {
                //// console.log('onload progress timer',Date.now())	
                ...
                function Assessmentclick() {
                    ...
                    if (!(i < glo_obj_len)) {
                        ...
                        //},500)
                        // smart check ending back to original state
                        document.querySelector("#btnFilter").click()
                        setTimeout(function(){
                            smart_Search_Checkbox = $("#chkSmartSearch")[0].checked
                            if (smart_checked){
                                console.log('checking smart search again back to original.')
                                smart_checked = false;
                                document.querySelector("#chkSmartSearch").click()
                            }else{
                                console.log('smart search already unchecked')                    
                        ...
                        if (data_obj.length > 0) {
                            setTimeout(function () {
                                document.querySelector("#pnModalBtn1").click();
                                //// console.log("assessment_submittion end close modal",Date.now());																						
                                setTimeout(function () {
                                    console.log('remove class');
                                    //// console.log("assessment_submittion end ",Date.now());
                                    $('.page-container').removeClass('visibile_no');
                                    $modal.removeClass('visibile_no')
                                    visiblility = 1;
                                    ...
                                    // console.log('data_obj final', data_obj);
                                    ...
                                    // console.log('postsubmission: ',postsubmittedRejected);
                                    ...
                                    // console.log("submit plan of care codes assessment: ", selected_text_obj)

                                    if (selected_text_obj.length > 0) {
                                        ...
                                        console.log("no plan of care, submission: ", data_obj)
                                        // athena.formatProcessedCodes(data_obj, 'success');
                                        if(data_obj.length > 0){
                                        $('button.select-item').removeClass('disabled');
                                        $('button.more-info').removeClass('disabled');
                                        ...
                                        makeRequest('POST', "https://myinfera.inferscience.com/api/post-submitted-rejected-data", postsubmittedRejected).then(function (data) {
                                            ...
                                            myObject.EncounterID = JSON.parse(patient_obj).LastEncId ?? null;	
                                            myObject.main_user = JSON.parse(patient_obj).main_user ?? null;                        
                                            myObject.ExtensionLaunch = "ecw";
                                            myObject.submitemr = JSON.parse(data);
                                            athena.ecwLog('Type:74',myObject);

                                        });
                                        athena.showMessages($messages, [
                                            'The selected item(s) were successfully submitted to your EMR.'
                                            ],
                                            'success'
                                        );
                                        athena.formatProcessedCodes(data_obj, 'success');
                                        // toggleData = true;
                                        localStorage.setItem('submission', "submission_completed");
                                        setTimeout(function(){                                    
                                            $messages = $('.messages', $('.container'));	
                                            $messages.removeClass('toast-msg')						
                                            $messages.empty();
                                            ...
                                            athena.showMessages($messages, [
                                                'The selected item(s) were not successfully submitted to your EMR.'
                                                ],
                                                'error'
                                            );
                                            setTimeout(function(){                                    
                                        ...
                                        // console.log("assessment_submittion end submission - completed ", Date.now());
                                        // t = setTimeout(athena.onloadTimer, 1000);
                                        /*
                                        makeRequest('POST', "https://myinfera.inferscience.com/api/post-submitted-rejected-data",postsubmittedRejected).then(function(data){
                                            //// console.log('data',data);
                    ...
                    function setValueTimer() {
                        try {
                            //// console.log("value try",data_obj[i]);
                            document.querySelector("#txtICDDescr").value = data_obj[i];
                            if (document.querySelector("#txtICDDescr").value == data_obj[i]) {
                                //// console.log("value entered");
                                document.querySelector("#txtICDDescr").value = data_obj[i];
                                t = setTimeout(Assessmentselection, 100);
                            }
                            if (document.querySelector("#txtICDDescr").value != data_obj[i]) {
                                //// console.log("value entered",document.querySelector("#txtICDDescr").value);
                            ...
                            console.log("Error while entering(setvaluetimer): ",err)
                            onloadfunction_count += 1;
                    ...
                    function Assessmentselection() {
                        ...
                        try {
                            console.log('Entered Assessmentselection')
                            document.querySelector("#txtICDDescr").dispatchEvent(keyboardEvent);
                        } catch (err) {
                            console.log("Error while assessmentvalue dispatch",err)
                        ...
                        function Submitclick() {
                            ...
                            let id_val = $('#AssessmentDataGrid1 > tbody > tr').attr('id');
                            // console.log("id_val", id_val);
                            let selector = '#' + id_val + ' > td:nth-child(10)';
                            // console.log(selector);

                            try {
                                if (!document.querySelector(selector)) {
                                    skipped_codes.push(data_obj[i])
                                    console.log("code is not present in ECW Assessment: ", data_obj[i])
                                    i = i + 1;
                                    // console.log("i", i);
                                ...
                                if (!(onloadfunction_count2 > 6)) {
                                    ...
                                    newRelicObject.category = newRelicLogCategory;
                                    newRelicObject.logTypeCategory = newRelicLogTypeCategory;
                                    newRelicObject.logType = "Ubiquity-KEY7";
			                        newRelicObject.customError = "During Submit Codes - selector is changed in the eCW" + patient_obj + "";
                                    newRelicObject.error = err;
                                    athena.athenaNewRelicLog(newRelicObject);
                                    // console.log('Submitclick timer function stopped looping Selector is changed');                                    
                                    //// console.log('Submitclick timer function stopped looping')
                                }
                            ...
                            setTimeout(function () {
                                if (document.querySelector(".bootstrap-dialog-footer-buttons")) {
                                    $('.bootstrap-dialog').addClass('visibile_no')
                                    console.log("popup raised")
                                    document.querySelector(".bootstrap-dialog-footer-buttons > .btn-lgrey").click()
                                }
                                //var submitText = document.querySelector("#AssessmentData").textContent;
                                //var matchesArray = submitText.match(/[A-Z][0-9][A-Z0-9](?:\.[A-Z0-9]{1,4})?/g);
                                //// console.log(matchesArray);
                                var matchesArray = [];
                                var submittedRowCount = document.querySelector("#AssessmentData > tbody").children.length;
                                for (let k = 1; k <= submittedRowCount; k++) {
                                    matchesArray.push(document.querySelector("#AssessmentData > tbody > tr:nth-child(" + k + ") > td:nth-child(3)").textContent)
                                    // console.log(" Codes are APPENDING TO ECW !!!!!")
                                }
                                // console.log(matchesArray);
                                assessmentCodesFinal = matchesArray;
                                athena.formatProcessedCodes(matchesArray, 'success');

                                if (matchesArray.includes(data_obj[i]) || matchesArray.includes(data_obj[i] + '0')) {
                                    console.log("Success pass check: ", data_obj[i]);
                                    i = i + 1;
                                    return Assessmentclick()
                                } else {
                                    if (onloadfunction_count3 > 3){
                                        // smart check ending back to original state
                                        document.querySelector("#btnFilter").click()
                                        setTimeout(function(){
                                            smart_Search_Checkbox = $("#chkSmartSearch")[0].checked
                                    ...
                                    onloadfunction_count3 += 1;
                                    console.log("assessment selection function retry");
                                    t = setTimeout(Assessmentselection, 100);
                                }
    ...
    submitPlanOfCare: function (selected_text_array, postsubmittedRejected, data_obj) {
        ...
        function checkEveryModalTimer(counter = 0, maxTimer) {
            // console.log(counter, maxTimer);
            if ((counter > maxTimer) || (document.querySelector("#mainPNDialog")) || (document.querySelector("#treatmentClinicalNotes"))) {
                // console.log('plan of care modal end', Date.now())
        ...
        function checkEveryInsideModalTimer(counter = 0, maxTimer) {
            // console.log(counter, maxTimer);
            if ((counter > maxTimer) || (document.querySelector("#treatmentClinicalNotes"))) {
                // console.log('plan of care inside modal end', Date.now())
                t = setTimeout(beforeAddNotes, 100);
                return false;
            } else {
                setTimeout(function () {
                    //// console.log('counter1'+counter,maxTimer);
                    if (document.querySelector("#pn")) {
                        // console.log("hided the inside modal");
                        $("#pn").addClass("visibile_no");
                    }
                    if (document.querySelector("#treatmentClinicalNotes > div.modal-dialog.ui-draggable")) {
                        $("#treatmentClinicalNotes > div.modal-dialog.ui-draggable").addClass('visibile_no')
                    ...
                    counter += 100
                    checkEveryInsideModalTimer(counter, maxTimer);
                }, 200);
            }
        ...
        function onPlanOfCareLoadModal() {
            ...
            //checkEveryModalTimer(0,500)
            try {
                //// console.log("add notes click",Date.now())
                document.querySelector("#treatmentasses > div:nth-child(1) > div > i.icon.addnotes-custom-icon.show-notes-div-data.mr5.mt3").click() // add notes
                checkEveryInsideModalTimer(0, 900)
                //t = setTimeout(beforeAddNotes, 500);
                /*
                let i=0;
                t = setTimeout(onAddNotesModal, i);
                function onAddNotesModal(){
                    //// console.log("add notes modal",Date.now())
                    //checkModalTimer(0,100)						
                    //// console.log("waited for ms",i)
                    if(document.querySelector("#treatmentClinicalNotes")){
                        // console.log("success")
                        i = -1;
                        t = setTimeout(beforeAddNotes, 100);
                    }else{
                ...
                */
                ...
                if (!(onloadfunction_count > 6)) {
                    ...
                    newRelicObject.category = newRelicLogCategory;
                    newRelicObject.logTypeCategory = newRelicLogTypeCategory;
                    newRelicObject.logType = "Ubiquity-KEY14";
                    newRelicObject.customError = "eCW: Plan of care selector is change " + patient_obj + "";
                    newRelicObject.error = err;
                    athena.athenaNewRelicLog(newRelicObject);
                    // console.log('onPlanOfCareLoadModal function stopped looping')
                    
                    //document.querySelector("#pnModalBtn1").click();
                    t = setTimeout(submitplan, 100);
                    //// console.log('assessment value timer function stopped looping')
                }
        ...
        // main function to add notes
        function beforeAddNotes() {
            // console.log("success end 2", Date.now())
            //checkEveryModalTimer(0,200)
            try {
                //checkEveryModalTimer(0,200)
                let ass_len = document.querySelector("#treatmentClinicalNotes_assessmentsList > ul").children.length;
                // console.log(ass_len, selected_text_array)
                let i = 0,
                    j = 0; // i for array & j for looping
                ...
                function loadingAddNotes() {
                    ...
                    //// console.log(((i<selected_text_array.length) && (j<ass_len)))
                    if (!((i < selected_text_array.length) && (j < ass_len))) {
                                                // setTimeout(function(){
                        $modal = $('#loading-modal');
                        $modal.removeClass('display');
                        $spinner = $('.loading-spinner', $modal);
                        ...
                        document.querySelector("#treatmentClinicalNotes > div.modal-dialog.ui-draggable > div > div.modal-footer.lb-foot.pd510 > button").click();
                        document.querySelector("#pnModalBtn1").click();
                        ...
                        // athena.formatProcessedCodes(data_obj, 'success');
                        if(selected_text_array.length > 0){
                            athena.showMessages($messages, [
                                'The selected item(s) were successfully submitted to your EMR.'
                                ],
                                'success'
                        ...
                        makeRequest('POST', "https://myinfera.inferscience.com/api/post-submitted-rejected-data", postsubmittedRejected).then(function (data) {
                            console.log('postplan of care submitteddata');
                        });
                        ...
                        try {
                            ...
                            //// console.log("while loop else",Date.now())
                            let ass_selector = '#treatmentClinicalNotes_assessmentMenuList_item' + j + ' > a';
                            let ass_sel_text = document.querySelector(ass_selector).text;
                            let ass_sel_code = ass_sel_text.match(/[A-Z][0-9][A-Z0-9](?:\.[A-Z0-9]{1,4})?/g);
                            //// console.log(ass_selector)
                            //// console.log(ass_sel_code[0])
                            //// console.log(selected_text_array[i][0])
                            if (ass_sel_code[0] == selected_text_array[i][0]) {
                                // console.log("if looping");
                                document.querySelector(ass_selector).click();
                                t = setTimeout(onAddNotes, 2000)

                                function onAddNotes() {
                                    //// console.log("text editor edit")
                                    $("#treatmentClinicalNotes_notesEditor > div:nth-child(1) > iframe")[0].contentDocument.childNodes[1].childNodes[1].innerText = selected_text_array[i][1]

                                    i++;
                                    j = 0; // increment for submit plan of care array
                                    return loadingAddNotes();
                                }

                            } else {
                                //// console.log("else looping")
                                j++ // increment for looping
                                return loadingAddNotes();
                            }
                        } catch (err) {
                            onloadfunction_count += 1;
                            ...
                            if (!(onloadfunction_count > 6)) {
                                ...
                                newRelicObject.error = err;
                                athena.athenaNewRelicLog(newRelicObject);
                                // console.log('loadingAddNotes function stopped looping')                                
        ...
        // main start point
        athena.loaderLoading(false);
        t = setTimeout(submitplan, 1000);

        function submitplan() {
            // console.log("submit plan start", Date.now())
            for (let i = 0; i < 100; i++) {
                let selector1 = '#pnData > a:nth-child(' + i + ') > font > b';
                let val = document.querySelector(selector1);
                if (val) {
                    //// console.log('value content: ',val)
                    if (val.textContent == 'Treatment:') {
                        // console.log('treatment', Date.now())
                        document.querySelector(selector1).click();
                        //checkEveryModalTimer(0,900)
                        //document.querySelector("#pnData > a:nth-child(57) > font > b").click() // click
                        checkEveryModalTimer(0, 900);
        ...
        /*
        //document.querySelector("#treatmentClinicalNotes_assessmentMenuList_item1 > a") // left side sections of codes
        $("#treatmentClinicalNotes_assessmentMenuList_item1 > a").click() // left side sections click of codes
    	
        document.querySelector("#treatmentClinicalNotes_assessmentsList > ul").children.length  // length of assessment section
    	
        document.querySelector("#treatmentClinicalNotes_assessmentMenuList_item1 > a").text // test retrieve of sections
    	
        $("#treatmentClinicalNotes_notesEditor > div:nth-child(1) > iframe")[0].contentDocument.childNodes[1].childNodes[1].innerText = "something"  // body text 
        //innerText < scrollingElement < contentDocument < previousSibling < nextSibling < $("#treatmentClinicalNotes_notesEditor > div:nth-child(1) > iframe")[0]
    	
        document.querySelector("#treatmentClinicalNotes > div.modal-dialog.ui-draggable > div > div.modal-footer.lb-foot.pd510 > button").click()  // close or save
    	
        document.querySelector("#pnModalBtn1").click(); // close the modal
        */
    ...
    showMessages: function ($container, messages, status, scrollIntoView) {
        if ($container[0] == $('.messages', $('.container'))[0]){
            $container = $('.messages', $('.container'));    
            $container.empty();
            $container.addClass('toast-msg')
        ...
        // console.log('showMessages', messages, status);
        var alertClass = '';
