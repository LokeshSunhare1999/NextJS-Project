...
import { usePostSendSms, usePutUpdateTrueId } from '../../apis/queryHooks';
...
const VerificationHeader = ({
  ...
  const [isDocumentUploadOpen, setDocumentUploadOpen] = useState(false);
  const dropdownList = possibleStates?.map(
    (item) => VERIFICATION_STATUS_MAP?.POSSIBLE_STATES?.[item?.state],
  );
  const { hasPermission } = usePermission();
  const {
    mutateAsync: updateTrueIdMutation,
    status: updateTrueIdStatus,
    isError: isUpdateTrueIdErr,
    error: updateTrueIdErr,
  } = usePutUpdateTrueId();

  const {
    mutateAsync: postSendSms,
  ...
  useEffect(() => {
    if (isUpdateTrueIdErr)
      enqueueSnackbar(
        `Failed to update status. error : ${updateTrueIdErr?.message}`,
        {
          variant: 'error',
        },
      );
  ...
  useEffect(() => {
    if (isSendSmsError)
      enqueueSnackbar(`Failed to send SMS. error : ${sendSmsErr?.message}`, {
        variant: 'error',
      });
  }, [isSendSmsError]);
  ...
  const handleUpdateStatus = async (message, customProps = {}) => {
    ...
    updateModal(
      <RemarksModal
        ...
        text={
          modalStatus === 'OPS_REJECTED'
            ? 'Confirm Action : Reject'
            : 'Confirm Action : Verify'
        }
        primaryCtaBgColor={
          modalStatus === 'OPS_REJECTED' ? '#ED2F2F' : '#32B237'
        }
        primaryCtaText={modalStatus === 'OPS_REJECTED' ? 'Reject' : 'Verify'}
      />,
    );

    await updateTrueIdMutation({
      customerId,
      [statusKey]: { verificationStatus: customProps.status },
      remarks: {
        message,
  ...
  const triggerSendSms = async () => {
    await postSendSms({
      customerId,
      verificationStatus,
      workflow: VERIFICATION_PAGE_INFO[pageRoute]?.WORKFLOW,
    });
    enqueueSnackbar(`SMS was sent successfully`, {
      variant: 'success',
    });
    refetchCustomerData();
  };
  ...
  const pageInfo = VERIFICATION_TEXTS[pageRoute];
  return (
    <Wrapper>
      <Left>
      ...
      <FlexContainer $justifyContent="space-between">
        ...
        {pageInfo?.TITLE.includes('PAN') && (
          <CustomCTA
            onClick={handleUploadPanClick}
            title="Upload PAN"
            showSecondary={true}
            loadingColor={'#141482'}
            color={'#141482'}
        ...
        {showNotificationButton ? (
          <FlexContainer $justifyContent="flex-end">
            <P $fontSize={'14px'} $fontWeight={'400'} $lineHeight={'normal'}>
              {pageInfo?.HEADER_STRING}
            </P>
            <CustomCTA
              onClick={() => triggerSendSms()}
              title="Send SMS"
              showSecondary={true}
              isLoading={sendSmsStatus === 'pending'}
              loadingColor={'#141482'}
              color={'#141482'}
              bgColor={'#ffffff'}
              border={'1px solid #141482'}
